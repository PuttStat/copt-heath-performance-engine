...
export function CourseMap({ hole, features, shots, onSelect }: { hole: CourseHole; features: CourseFeature[]; shots: MappedShot[]; onSelect: (coordinate: Coordinate) => void }) {
  ...
  return <div className="course-map-shell"><svg className="course-map" viewBox="0 0 1000 700" role="img" aria-label={`Hole ${hole.hole_number} course map. Tap where the ball finished.`} onClick={click}>
    ...
    {ordered.filter(f=>f.geometry.length>1).map(feature=><polygon key={feature.id} points={feature.geometry.map(point=>{const p=project(point);return `${p.x},${p.y}`}).join(" ")} fill={colour(feature.feature_type)} fillOpacity={/Boundry|Generic/i.test(feature.feature_type)?.3:.92} stroke="rgba(255,255,255,.18)" strokeWidth="2"/>)}
    {shots.map(shot=>{const a=project({latitude:shot.startLatitude,longitude:shot.startLongitude}),b=project({latitude:shot.endLatitude,longitude:shot.endLongitude});return <line key={`${shot.id}-line`} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="#f7f2e8" strokeWidth="4" strokeDasharray="10 8" opacity=".85"/>})}
