...
export default function QuickRound() {
  ...
  const saveRound = async () => { const id = crypto.randomUUID(); queueRound({ id, recordedAt: new Date().toISOString(), bands }); localStorage.removeItem("vgp-quick-round"); setBands(initial); setSaved(true); const result = await syncQueuedRounds(); setSyncMessage(result.synced ? "Round securely synced." : "Round is safely queued and will sync when a connection is available."); };
  return <AppShell active="rounds"><header className="page-heading"><div><p className="eyebrow">Round capture</p><h1>Quick round</h1><p>Record only opportunities and successful outcomes. Empty means not entered; zero means it happened with no successes.</p></div><a className="secondary-action" href="/rounds/detailed">Detailed hole-by-hole <span>→</span></a></header>
    {saved && <div className="success-banner" role="status">{syncMessage || "Round saved to this device and ready to sync."}</div>}
